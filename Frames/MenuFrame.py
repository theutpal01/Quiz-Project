...
class MenuFrame(tk.Frame):
    ...
    def __init__(self, parent: tk.Frame, controller: tk.Tk):
        ...
        resBtn = tk.Button(headFrame, text="Get Your Result", font=(FONT_FAM, FONT_SIZE), padx=10, pady=5, background=S_COL, foreground=TXT_COL, relief=tk.FLAT, bd=0, width=20, command=controller.showResult)
        resBtn.pack(padx=15, pady=15, anchor=tk.W)
        resBtn.bind('<Enter>', controller.hoverBtn)
        resBtn.bind('<Leave>', controller.unhoverBtn)
    
        logoutBtn = tk.Button(headFrame, text="Logout Quiz App", font=(FONT_FAM, FONT_SIZE), padx=10, pady=5, background=S_COL, foreground=TXT_COL, relief=tk.FLAT, bd=0, width=20, command=controller.logout) 
