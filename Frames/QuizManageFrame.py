...
class QuizManageFrame(tk.Frame):
    ...
    def __init__(self, parent: tk.Frame, controller: tk.Tk):
        ...
        self.previewBtn = tk.Button(leftFrame, text="Start Preview", font=(FONT_FAM, VSM_FONT_SIZE), command=lambda: self.previewText(controller), width=22, pady=5, background=S_COL, foreground=TXT_COL, relief=tk.FLAT, bd=0)
        ...
        self.delBtn.place(in_=leftFrame, relx=0.5, rely=0.83, anchor=tk.CENTER)
        ...
        self.opt1 = tk.Entry(opt1Frame, font=(FONT_FAM, VSM_FONT_SIZE), width=22, disabledforeground=P_COL, disabledbackground=TXT_COL)
        ...
        self.opt2 = tk.Entry(opt2Frame, font=(FONT_FAM, VSM_FONT_SIZE), width=22, disabledforeground=P_COL, disabledbackground=TXT_COL)
        ...
        self.opt3 = tk.Entry(opt3Frame, font=(FONT_FAM, VSM_FONT_SIZE), width=22, disabledforeground=P_COL, disabledbackground=TXT_COL)
        ...
        self.opt4 = tk.Entry(opt4Frame, font=(FONT_FAM, VSM_FONT_SIZE), width=22, disabledforeground=P_COL, disabledbackground=TXT_COL)
        ...
        self.ans = tk.Entry(inputFrame, font=(FONT_FAM, VSM_FONT_SIZE), width=46, disabledforeground=P_COL, disabledbackground=TXT_COL)
        self.ans.pack(pady=2, fill=tk.X)
        inputFrame.pack()

        submitFrame = tk.Frame(rightFrame, padx=20, background=P_COL)
        self.saveBtn = tk.Button(submitFrame, text="Save Question", font=(FONT_FAM, VSM_FONT_SIZE), command=lambda: controller.saveQuest(self.getData()), width=21, pady=5, background=S_COL, foreground=TXT_COL, relief=tk.FLAT, bd=0)
        ...
        backBtn = tk.Button(submitFrame, text="Go Back", font=(FONT_FAM, VSM_FONT_SIZE), width=21, pady=5, background=S_COL, foreground=TXT_COL, relief=tk.FLAT, bd=0, command=lambda: controller.showFrame("admin"))
        backBtn.pack(side=tk.RIGHT, padx=(0, 5), pady=5)
        backBtn.bind('<Enter>', controller.hoverBtn)
        backBtn.bind('<Leave>', controller.unhoverBtn)
        submitFrame.pack(pady=(30, 0))

        self.multiSaveBtn = tk.Button(rightFrame, text="Save Multiple Questions", font=(FONT_FAM, VSM_FONT_SIZE), width=45, pady=5, background=S_COL, foreground=TXT_COL, relief=tk.FLAT, bd=0, command=lambda: self.saveMultiple(controller))
        self.multiSaveBtn.pack(padx=5, pady=(0, 5))
    ...
    def previewText(self, controller):
        ...
        if selected is not None and not QuizManageFrame.Preview:
            ...
            controller.box.showBox("Warning", "Please first select a question to preview it.", "w")
                ...
                if controller.box.showYesNoBox("Want to continue?", "Deleting a question will clear the data of users who have gven the quiz. Do you wish to continue?"):
                    ...
                    if info[0] == "Success":
                        if controller.database.clearTable(RESULT_TABLE)[0] == "Success":
                            ...
                            if info[0] == "Success":
                                controller.box.showBox("Info", "The question haas been deleted successfully!", "i")
                    else:
                        controller.box.showBox("Error", "Something went wrong! Please restart the program.", "e")
